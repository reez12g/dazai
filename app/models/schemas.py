...
class SentenceMaterial(BaseModel):
    ...
    class Config:
        schema_extra = {
            "example": {"text": "今日の天気は", "response_url": "https://example.com/webhook"}
        }
...
class ResponseMessage(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"text": "信じられているから走るのだ。少し考えてみよう。"}}
...
class TaskResponse(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"status": "Text generated and sent successfully"}}
...
class StyleTransferRequest(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"text": "今日はとても良い天気です。", "target_style": "meiji"}}
...
class StyleTransferResponse(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"transformed_text": "今日はとても良い天気であります。"}}
...
class SummarizationRequest(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"text": "長い文章をここに入力します。この文章は要約されます。", "max_length": 50}}
...
class SummarizationResponse(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"summary": "長い文章の要約。"}}
...
class SentimentAnalysisRequest(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"text": "この映画はとても面白かったです。"}}
